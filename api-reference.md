...
#### `registerWallet(blockchain, WalletManager, config)` [#registerwalletblockchain-walletmanager-config]
...
#### `registerProtocol(chain, label, Protocol, config?)` [#registerprotocolchain-label-protocol-config]
...
#### `useIndexer(config)` [#useindexerconfig]
...
#### `registerTools(tools)` [#registertoolstools]
...
#### `registerToken(chain, symbol, token)` [#registertokenchain-symbol-token]
...
### `getMaxSpendableBtc` [#getmaxspendablebtc]
...
### `quoteSendTransaction` [#quotesendtransaction]
...
## Indexer Tools [#indexer-tools]

*Requires: `useIndexer()`*

### `getIndexerTokenBalance` [#getindexertokenbalance]
...
### `getSupportedCryptoAssets` [#getsupportedcryptoassets]
...
### `getSupportedFiatCurrencies` [#getsupportedfiatcurrencies]
...
### `parseAmountToBaseUnits(amount, decimals)` [#parseamounttobaseunitsamount-decimals]
...
### `formatBaseUnitsToAmount(baseUnits, decimals)` [#formatbaseunitstoamountbaseunits-decimals]
