...
<Steps>
  <Step>
    #### Client requests a resource [#client-requests-a-resource]

    A standard HTTP request. `GET`, `POST`, whatever your API expects.
  </Step>

  <Step>
    #### Server responds with 402 Payment Required [#server-responds-with-402-payment-required]
  ...
  <Step>
    #### Client signs a payment [#client-signs-a-payment]
  ...
  <Step>
    #### Client retries with payment header [#client-retries-with-payment-header]

    The signed payload goes in the `X-PAYMENT` header on the same request.
  </Step>

  <Step>
    #### Facilitator verifies [#facilitator-verifies]
  ...
  <Step>
    #### Server returns the resource [#server-returns-the-resource]
...
## How to Use x402 With WDK [#how-to-use-x402-with-wdk]
...
## Client: Paying for Resources [#client-paying-for-resources]
...
<Steps>
  ...
  <Step>
    #### Register with x402 [#register-with-x402]
  ...
  <Step>
    #### Make a paid request [#make-a-paid-request]
...
### Getting USD₮0 on Plasma or Stable [#getting-usd0-on-plasma-or-stable]
...
#### Bridge USD₮ from Ethereum to Plasma / Stable [#bridge-usd-from-ethereum-to-plasma--stable]

<Steps>
  <Step>
    #### Create wallet and bridge protocol [#create-wallet-and-bridge-protocol]
  ...
  <Step>
    #### Approve and get a quote (recommended) [#approve-and-get-a-quote-recommended]
  ...
  <Step>
    #### Execute the bridge [#execute-the-bridge]
...
## Server: Accepting Payments (Hosted Facilitator) [#server-accepting-payments-hosted-facilitator]
...
<Steps>
  <Step>
    #### Derive your receiving address [#derive-your-receiving-address]
  ...
  <Step>
    #### Create the facilitator client [#create-the-facilitator-client]
  ...
  <Step>
    #### Configure payment middleware [#configure-payment-middleware]
...
### Multi-Chain (Plasma + Stable) [#multi-chain-plasma--stable]
...
## Server: Self-Hosted Facilitator (In-Process) [#server-self-hosted-facilitator-in-process]
...
<Steps>
  <Step>
    #### Create the facilitator signer [#create-the-facilitator-signer]
  ...
  <Step>
    #### Initialize the facilitator [#initialize-the-facilitator]
  ...
  <Step>
    #### Wire into Express [#wire-into-express]
