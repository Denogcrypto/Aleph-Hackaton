...
(function () {
  ...
  window.PaygentTheme = {
    ...
    renderToggle(container) {
      ...
      container.innerHTML = `
        <button onclick="window.PaygentTheme.toggleTheme()" 
                ...
                class="flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-900/90 border border-slate-300 dark:border-slate-800 hover:border-teal-500 dark:hover:border-teal-500 text-slate-700 dark:text-slate-300 transition-all group">
    ...
    updateTogglesUI() {
      document
        .querySelectorAll(
          "[data-theme-toggle], #themeToggleContainer, #topThemeToggle",
        )
        .forEach((el) => {
